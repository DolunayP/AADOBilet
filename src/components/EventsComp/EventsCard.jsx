    ...
    return (
        <>
            <div className='w-[24%] text-gray-100 group-hover:opacity-0 sm:group-hover:opacity-95 shadow-[10px_14px_16px_-2px_rgba(0,0,0,0.3)] gap-2 flex flex-col bg-[#32847a] rounded-lg py-8 ps-6 pe-2'>
                ...
                <p className='text-start'><span className='font-bold'>Date:</span> {eventDate}</p>

                <a onClick={() => getDetails(event.id)} className='cursor-pointer shadow-xl mt-4 me-4 py-3 px-6 bg-black text-white rounded-full hover:scale-95 hover:text-[#67f7e6] transition-all duration-500'>Buy a Ticket</a>
                <a onClick={() => getDetails(event.id)} className='cursor-pointer me-4 py-3 px-6 bg-black text-white rounded-full hover:scale-95 hover:text-[#67f7e6] transition-all duration-500'>See Details</a>
